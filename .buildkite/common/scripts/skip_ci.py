...
class CiDecision:
    ...
    def _device_from_run(run: dict[str, dict[str, bool]]) -> CiDevice:
        return CiDevice(
            **{
                f"skip_{platform}_{level}": not run[platform][level]
                for platform in PLATFORMS
                for level in ("l2", "l3")
            }
        )
...
def _resolve_diff_range() -> str | None:
    ...
    is_pr = os.environ.get(
        "BUILDKITE_PULL_REQUEST", "false"
    ) != "false" and os.environ.get("BUILDKITE_PULL_REQUEST", "")
...
def resolve_ci_context_from_git() -> CiContext:
    ...
    return CiContext(
        decision=decision, changed_files=changed_files, diff_range=diff_range
    )
...
def _is_skip_testcase_change(file_path: str, *, diff_range: str) -> bool:
    ...
    match = re.search(
        r"^pytestmark\s*=\s*\[(.*?)^\s*\]", content, re.MULTILINE | re.DOTALL
    )
    if match is not None and _PYTESTMARK_SKIP_RE.search(match.group(1)) is not None:
        _log(
            f"skip-mark-only: new test file with module-level pytest.mark.skip: {file_path}"
        )
        return True
    ...
    _log(
        f"skip-mark-only: new test file without module-level pytest.mark.skip: {file_path}"
    )
    return False


def _classify_changed_files_into_buckets(
    changed_files: list[str], *, diff_range: str | None
) -> DiffBuckets:
    ...
    for file_path in changed_files:
        ...
        if (
            ...
            or file_path == "mkdocs.yml"
        ):
            buckets.docs.append(file_path)
            continue
        path = Path(file_path)
        if path.suffix == ".py" and path.parts and path.parts[0] == "tests":
            if diff_range is not None and _is_skip_testcase_change(
                file_path, diff_range=diff_range
            ):
...
def _format_yaml_gated_message(buckets: DiffBuckets, decision: CiDecision) -> str:
    ...
    run_targets = [
        ...
        for platform in PLATFORMS
        for level in ("l2", "l3")
        if decision.is_run(platform, level)
    ]
    skip_targets = [
        f"{platform}/{level}"
        for platform in PLATFORMS
...
def resolve_ci_decision(
    ...
    *,
    ...
    if buckets.has_other_changes or not buckets.has_skip_ci_scope_changes:
        return _finish(
            CiDecision(), "product or non-whitelisted changes present; run normal CI"
        )
    ...
    if not buckets.has_l23_yaml_changes and not buckets.has_l45_yaml_changes:
        if buckets.has_skip_test_changes and diff_range is None:
            ...
            message = (
                "doc/skip-mark changes did not qualify for skip-all; run normal CI"
            )
